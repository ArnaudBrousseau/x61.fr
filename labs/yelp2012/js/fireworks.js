/**
 * Script adapted from CreativeJS tutorial, by Paul Lewis.
 */
...
var Fireworks = (function() {
  ...
  function initialize() {
    ...
    mainBackground = document.getElementById("nightsky");
...
var FireworkExplosions = {
  ...
  /**
   * Woohoo! Yelp's pattern :)
   */
  yelp: function(firework) {
    /* Determines the pattern */
    function isYelpish(angle) {
      var pi = Math.PI;
      if (angle > pi/12 && angle < pi/3) {
        return 1; //small branch
      }
      if (angle > pi/2 && angle < 2*pi/3) {
        return 1; //small branch
      }
      if (angle > 10*pi/12 && angle < 13*pi/12) {
        return 1; //small branch
      }
      if (angle > 5*pi/4 && angle < 3*pi/2) {
        return 2; //THE fifth star :)
      }
      if (angle > 5*pi/3 && angle < 23*pi/12) {
        return 1; //small branch
      }
      return 0; //no sparks this time bro :( Play again
    }

    var count = 60 + Math.floor(Math.random()*100);
    var angle = (Math.PI * 2) / count;
    
    while(count--) {

      var randomVelocity = 0 + Math.random() * 10;
      var particleAngle = count * angle;
      var yelpishDegree = isYelpish(particleAngle); 
      if (yelpishDegree) {
        Fireworks.createParticle(
          firework.pos,
          null,
          {
            x: Math.cos(particleAngle) * randomVelocity*yelpishDegree,
            y: Math.sin(particleAngle) * randomVelocity*yelpishDegree
          },
          firework.color,
          true);
      }
